...
contract GettersAndDerivers is ConsiderationBase {
     ...
     *      caller.  
     根据OrderParameters组装OrderComponents结构体类型， 以内联汇编的方式计算 OrderComponents 所对应的Orderhash（参考eip712的结构化hash方式）
     *
    ...
    function _deriveOrderHash(
        ...
        ); // 计算orderHash时，只截取挂单者挂单时的ConsiderationItem[]、去掉当前接单者附加的部分
     ...
     *      using a corresponding conduit key. 根据conduitKey计算对应的Conduit合约地址。
     *
        ...
        returns (address conduit)
    {
        ...
        assembly {
            ...
            mstore(0, or(MaskOverByteTwelve, conduitController)) //32字节= 前11个字节为0 + 1个字节的0xff + 20字节的controller地址

            // Place the conduit key in the next region of scratch space.
            mstore(OneWord, conduitKey) // 追加salt

            // Place conduit creation code hash in free memory pointer location.
            mstore(TwoWords, conduitCreationCodeHash) //追加合约字节码hash

            // Derive conduit by hashing and applying a mask over last 20 bytes. 计算Conduit合约地址
            conduit := and(
            ...
            // Restore the free memory pointer. 重置空闲内存指针
