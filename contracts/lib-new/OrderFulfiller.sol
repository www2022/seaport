    ...
    AmountDeriver
{
     ...
     * @return A boolean indicating whether the order has been fulfilled.
     作用： 验证订单参数、修改订单status、计算实际能够接单的token数量并完成token transfer
     */
    function _validateAndFulfillAdvancedOrder(
        ...
        (bytes32 orderHash, uint256 fillNumerator, uint256 fillDenominator) = _validateOrderAndUpdateStatus( //对于orderHash指定的订单，计算本次实际能够接单的份额：newNumerator/newDenominator
            advancedOrder,
            criteriaResolvers,
            true,
            priorOrderHashes
        ...
        _applyCriteriaResolvers(advancedOrders, criteriaResolvers); // 根据criteriaResolvers更新advancedOrders中的 identifierOrCriteria字段：由Merkle Root替换为真实的tokenId

        // Retrieve the order parameters after applying criteria resolvers.
        OrderParameters memory orderParameters = advancedOrders[0].parameters;

        // Perform each item transfer with the appropriate fractional amount.
        _applyFractionsAndTransferEach( // 完成token的transfer。并修改了orderParameters的内存空间，改变了offer和consideration
            orderParameters,
            fillNumerator,
            fillDenominator,
            fulfillerConduitKey,
        ...
        _emitOrderFulfilledEvent( // 触发OrderFulfilled事件
            orderHash,
            orderParameters.offerer,
            orderParameters.zone,
            recipient,
            orderParameters.offer, //_applyFractionsAndTransferEach中将offer变得和SpentItem[]相似
            orderParameters.consideration //_applyFractionsAndTransferEach中将offer变得和RecivedItem[]相似
        );
     ...
     *                            Consideration.
     * @param recipient           The intended recipient for all received items.
     // 执行orderParameters中的offer和consideration，完成各种token的transfer
     */
    function _applyFractionsAndTransferEach(
         ...
         *  复用OfferItem的内存空间，保存ReceivedItem的信息：
        ...
        unchecked {
            ...
            function(OfferItem memory, address, bytes32, bytes memory)
                internal _transferOfferItem; // _transferOfferItem是一个function类型变量，在下方{}中进行赋值
        ...
        _triggerIfArmed(accumulator); //批量执行accumulator中累加的transfer任务
     ...
     * @return advancedOrder The new advanced order.
      将Order类型 转为 AdvancedOrder类型， 不支持订单部分执行
     */
     ...
     * @return advancedOrders The new array of partial orders.
     【批量】将Order类型 转为 AdvancedOrder类型， 不支持订单部分执行
     */
