...
contract CriteriaResolution is CriteriaResolutionErrors {
     ...
     *                           that no proof needs to be supplied.
     根据criteriaResolvers，替换advancedOrders各个订单中的itemType及 identifierOrCriteria
     */
    function _applyCriteriaResolvers(
        ...
        unchecked {
            ...
            // 1. 根据criteriaResolvers数组，替换对应的AdvancedOrder中xxItem的itemType（由ERC721_WITH_CRITERIA替换为ERC721）、并将xxItem的identifierOrCriteria设置为criteriaResolver中指定的tokenId
            for (uint256 i = 0; i < totalCriteriaResolvers; ++i) {
                ...
                if (identifierOrCriteria != uint256(0)) {
                    ...
                    _verifyProof(
                        criteriaResolver.identifier, //leaf
                        identifierOrCriteria, //root
                        criteriaResolver.criteriaProof //proof
                    );
                }
            }

            // Iterate over each advanced order.
            // 2. 上边的for循环替换完成后，再依次遍历advancedOrders中的Consideration[]和Offer[],看其中的各个ItemType中是否还有。。。。_WITH_CRITERIA类型，如果有则直接revert
