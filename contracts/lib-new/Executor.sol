...
contract Executor is Verifiers, TokenTransferrer {
    ...
    function _transferIndividual721Or1155Item(
        ...
        if (conduitKey != bytes32(0)) {
            ...
            // 调用conduitKey指定的Conduit合约的 execute(ConduitTransfer[] calldata transfers) 完成token transfer
            assembly {
                ...
                // Write the offset to the ConduitTransfer array in memory.  参考ABI编码：数组类型的变量--偏移量+ 数组长度+各个元素
                mstore(
                    add(
                        callDataOffset,
                        Conduit_execute_ConduitTransfer_offset_ptr
                    ),
                    Conduit_execute_ConduitTransfer_ptr // 数组偏移量为0x20，即当前32个字节
                )
                ...
                mstore(
                    ...
                    Conduit_execute_ConduitTransfer_length // 数组长度：1
                )
                // 接下来，依次写入ConduitTransfer结构体的六个字段
     ...
     *                    against a given conduit in a single call.
     // accumulator的组成：
    // 1. armed状态时：第一个32字节： "armed" status-->0x40 + 第二个32字节：conduitKey + 4个字节：ConduitInterface.execute.selector+ execute()的结构体数组参数（数组偏移量+数组长度+数组各个元素）
    // 2. disarmed状态时：accumulator=0x20（此时，accumulator.length=0x20 因为，调用accumulator.length时，读取的是accumulator第一个32字节、与实际bytes长度无关、可人为设置）
     */
    function _transferERC20(
        ...
        // 如果accumulator中指定的conduitKey和入参中的不同，则根据accumulator调用Conduit合约的execute（参数由accumulator决定）；否则就追加到accumulator中、到最后一起执行，这样只需要去调用一次Conduit执行批量transfer
        ...
        if (conduitKey == bytes32(0)) {
            ...
            // 将当前参数指定的transfer 相关参数追加到变量accumulator中（本函数中 并不会执行！！）
            _insert(
                conduitKey,
                accumulator,
    ...
    function _transferERC721(
        ...
        // Trigger accumulated transfers if the conduits differ.
        _triggerIfArmedAndNotAccumulatable(accumulator, conduitKey); // 参考上方
     ...
     *                    signifies that no conduit should be used, with direct
     *                    approvals set on this contract.
     * 如果当前指定的Conduit和accumulator中累加的不一样，则批量执行accumulator中的transfer（ 后续，会将accumulator置空、然后将当前transfer任务添加进去、直到下一次批量transfer）
     */
    ...
    function _triggerIfArmed(bytes memory accumulator) internal {
        ...
        //accumulator长度是0x40 字节，即0x+16*4*2 位 16进制数字，取前32字节作为accumulatorConduitKey
        // Retrieve the current conduit key from the accumulator.
        bytes32 accumulatorConduitKey = _getAccumulatorConduitKey(accumulator);
     ...
     *  执行accumulator中指定的操作，最后将accumulator置为disarmed
    ...
    function _trigger(bytes32 conduitKey, bytes memory accumulator) internal {
        ...
        uint256 callDataOffset; // input data中，作为Conduit合约execute()入参的起始位置
        ...
        assembly {
            ...
            // 68 + items * 192
            callDataSize := add(
                Accumulator_array_offset_ptr, //0x44=0x4 函数选择器 +0x20 数组偏移量+ 0x20 数组长度 + 数组中各个元素（每个元素为structure、192字节）
                mul(
    ...
    function _callConduitUsingOffsets(
        bytes32 conduitKey,
        uint256 callDataOffset, // 内存中的偏移量
        uint256 callDataSize
    ) internal {
        ...
        returns (bytes32 accumulatorConduitKey)
    {
        ...
        assembly {
            accumulatorConduitKey := mload(
                add(accumulator, Accumulator_conduitKey_ptr) // 入参bytes的ABI编码是：0x20（偏移量）+bytes.length+bytes实际数据（右侧填充0补齐）
            )
        }
    }
     ...
     * 将本次transfer的各个参数（即调用Conduit合约的execute（）所需的参数）等追加到accumulator中
     # accumulator的组成： 第一个32字节 length or "armed" status + 第二个32字节：conduitKey + 4个字节：ConduitInterface.execute.selector+ execute()的结构体数组参数（数组偏移量+数组长度+数组各个元素）
     * @param conduitKey  A bytes32 value indicating what corresponding conduit,
     *                    if any, to source token approvals from. The zero hash
     *                    signifies that no conduit should be used, with direct
     *                    approvals set on this contract.
