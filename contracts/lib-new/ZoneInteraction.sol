...
contract ZoneInteraction is ZoneInteractionErrors, LowLevelHelpers {
    ...
    function _callIsValidOrder(
        ...
        // 调用zone合约的isValidOrder（）验证msg.sender是否为合法接单者
        // Perform minimal staticcall to the zone.
     ...
     * @param offerer           The offerer in question.
     * @param zone              The zone in question.
     作用：验证msg.sender是否可以接单，如果不能直接revert
     */
    function _assertRestrictedAdvancedOrderValidity(
        ...
        // orderType<1时，所有人都可以接单；orderType>1时，接单者必须是zone或者offerer、或者是调用zone合约的isValidOrder。。验证可以通过
